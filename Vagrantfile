...
# VM configuration
vm_hostname = 'docker-tomcat-libav'
vm_ram      = '512'
vm_cpus     = 2
vm_pae      = 'on'
...
Vagrant.configure("2") do |config|
  ...
  config.vm.define 'vm' do |node|
      ...
      #node.vm.provision "update-packages", type: "shell", inline: "DEBIAN_FRONTEND=noninteractive /usr/bin/apt-get update; /usr/bin/apt-get upgrade --force-yes -y; /usr/bin/apt-get autoremove -y", privileged: true
